...
const downloadImages = (listOfImages) => {
  const values = [];
  let count = 0
  for (let i=0; i<gettyImages.length; i+=1) {
    axios.get(listOfImages[i], {responseType: 'stream'})
    .catch((err) => {
      console.log('there was an error with the axios get request: ', err);
    })
    .then((response) => {
      let fileName = path.join(__dirname, `/images/treeHouseImage_${count}.jpg`);
